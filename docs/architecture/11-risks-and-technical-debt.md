...
| **A per-GPU budget row of `0` refuses every spec on that GPU**, while `routing.ServerGPUBudget.BudgetMB`'s doc comment says `0` means unconstrained | An operator who sets a budget to 0 intending "no limit" gets `not_permitted` for every model on that card | **Known contradiction between a comment and the agent's policy, left as found by the documentation pass.** The agent treats a *present* row with `budget_mb = 0` as a real zero budget and only an *absent* row as unconstrained; the portal's validation rejects only negative values, so 0 is reachable input. Until the two are reconciled, treat "no budget for this GPU" as **delete the row**. |
