...
| `OP_AGENT_RUNTIME_LOG_BUFFER_BYTES` | int (bytes) | How much of each managed model process's stdout+stderr the agent retains **in memory**, so an operator can read it after the fact. The buffer belongs to the spec, not the process, so a crashed model's output survives it. Below `65536` is raised to it. Never written to disk — this content can include prompt text | `1048576` (1 MiB) |
| `OP_AGENT_RUNTIME_LOG_BUFFER_TOTAL_BYTES` | int (bytes) | Ceiling on the **sum** of those buffers across every spec. The agent keeps at most `total / per-spec` of them, evicting the least-recently-written one nobody is watching. This is the number to size the agent's memory against | `16777216` (16 MiB) |
