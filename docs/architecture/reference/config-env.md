...
| `OP_AGENT_RUNTIME_ALLOWED_DIRS` | string list (comma-separated; **no flag form**) | Permitted `work_dir` prefixes for launch specs; **empty means any `work_dir`**. Lexical path-boundary containment — symlinks are not resolved (defence in depth, not the boundary). A bare entry already permits the directory **and its whole subtree**; a trailing whole-segment wildcard (`/srv/models/*` or `C:\models\*`) is an accepted, exactly-equivalent spelling of that subtree — it is **not** a glob (a glued `/srv/models*` stays literal and permits no sibling; a bare `*` permits nothing) | `` (any) |
| `OP_AGENT_RUNTIME_ALLOW_BINARY_DIRS` | bool (**no flag form**) | When on, each `RUNTIME_ALLOWED_BINARIES` entry's **parent directory** is treated as an allowed `work_dir` prefix without listing it in `RUNTIME_ALLOWED_DIRS` — convenient when a model's `work_dir` sits beside its binary. The auto-added dirs use the same subtree containment as explicit entries. **Footgun:** turning it on while `RUNTIME_ALLOWED_DIRS` is empty flips `work_dir` from "any is accepted" to "only the binary directories and their subtrees" (a spec that sets no `work_dir` still runs beside its binary either way) | `false` |
...
| `OP_AGENT_RUNTIME_ROUTER_BIND` | string | Bind host for the managed runtime's router port (the gateway supplies the router **port**, never the bind host). Empty derives the agent's mesh identity from the installed leaf in `CERT_DIR` — that directory alone is consulted, never `CERT_MODE` — else binds **all interfaces**, logged at Warn. Under `CERT_MODE=proxy`, `CERT_DIR` is mandatory, so the empty default resolves to the leaf's **mesh address**, not all interfaces and not loopback. **Resolved once at process start**: a boot before the first leaf install binds all interfaces until the agent restarts, and a SAN change on renewal does not move the router until then. The agent's own TLS-proxy path no longer depends on this value (it serves the router in process); where the router **listens** still does | `` (derive) |
| `OP_AGENT_RUNTIME_LOG_BUFFER_BYTES` | int (bytes; **no flag form**) | How much of each managed model process's stdout+stderr the agent retains **in memory**, so an operator can read it after the fact. The buffer belongs to the spec, not the process, so a crashed model's output survives it. Below `65536` is raised to it. Never written to disk — this content can include prompt text | `1048576` (1 MiB) |
| `OP_AGENT_RUNTIME_LOG_BUFFER_TOTAL_BYTES` | int (bytes; **no flag form**) | Ceiling on the **sum** of those buffers across every spec. The agent keeps at most `total / per-spec` of them, evicting the least-recently-written one nobody is watching. This is the number to size the agent's memory against | `16777216` (16 MiB) |
