...
- **Measured, not assumed:** with the natural primary key `(mapping_id, capability)`, adding a `LEFT JOIN … and capability = 'mtp'` to the resolver's per-request candidate query costs **≈ +6 µs** (17 µs → 23 µs at 5 000 mappings / 40 000 capability rows / 5 candidates per model; plan confirms one index seek per candidate row). Fetching *all* capabilities in that query costs +79 µs. Against an inference request of tens of milliseconds upward this is noise, so **performance is not a reason to keep a derived column** — and without that reason, a derived column only reintroduces the duplication and the sync.
...
- **`capability`** is an open vocabulary, not an enum: `vision`, `video`, `audio`, `tools`, `mtp` today; Ollama's `thinking`, `insert`, `embedding`, `image` and whatever ships next are rows, not migrations. That is the whole point. Go names constants only for the capabilities it reasons about; every other name is carried and displayed verbatim.
- **`source`** is `manual` | `vision_benchmark` | `llama_cpp_props` | `legacy` (a value inherited from a pre-78 column whose real origin is unknowable). PR B adds `ollama_show`.
- **`checked_at`** is `not null` — a row exists only because someone established it, so there is always a time. (`live_progress_checked_at`'s nullable-because-unknown case disappears with the row-presence model.)
...
Then dropped from `model_mappings`: `cap_vision`, `cap_video`, `cap_audio`, `cap_tools`, `cap_extra`, `capabilities_source`, `capabilities_checked_at`, `vision_capable`, `is_mtp`.
...
**Resolver / decision path.** `ActiveMappingsForModel` gains `left join model_mapping_capabilities mc on mc.mapping_id = m.id and mc.capability = 'mtp'` and drops the nine columns from its select list. `ModelMapping.IsMTP` and `ModelMapping.VisionCapable` are **removed from the struct** — deliberately not kept as sometimes-populated fields, which would be a footgun the moment a mapping loaded via `MappingByID` reported `false` while the resolver's reported `true`. Instead `MappingCandidate` carries `IsMTP bool` filled from the join, and `scoringRoute` reads it from there. **The scorer itself is untouched** — same `Route.IsMTP`, same `mtpBonus = 30.0`.

**Write-backs.** `writeBackRuntimeCapabilities` (+ `changedCapabilityVerdicts`, `capabilityResolution`, `resolvedCapabilities`, `applyWritten`) and `applyCapabilityWrite` (+ `diffCapabilityVerdicts`) are rewritten against rows. `syncVisionCapable` is **deleted outright** — with one source of truth there is nothing to sync, which is the cleanest possible answer to the operator's requirement. Everything else those functions guard is preserved: the runtimes cap, per-`spec_id` ownership resolution with the cross-server `slog.Warn` rejection, the per-sample `runtime_model_probe` feature gate, best-effort (never reject a telemetry sample), and compare-to-stored so an unchanged verdict issues no write. The precedence rule from §3 is the one new guard.
...
**Out of scope, on purpose:** `live_progress_support` stays a column. It is a capability by nature and folding it in would be consistent, but it sits on #52's dispatch decision (`Target.LiveProgressSupport` → `provider.wantsLiveProgress`) which has only just stabilised, and its three-layer rule is the one piece of capability logic with real request-shaping consequence. When it moves, it is one row in this table and no schema change.
...
- **Resolver:** the joined `mtp` verdict reaches `Route.IsMTP`; a mapping with no `mtp` row gets no bonus. Both revert-verified.
