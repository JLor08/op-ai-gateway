...
- Ingest additions in `ingestTelemetrySample`, all AFTER the store writes (the "evidence only after persistence" convention): (a) `s.RuntimeStatus.publish(serverID, statuses)`; (b) VRAM write-back. The sample carries `spec_id`, and the write itself is `UpdateRuntimeSpecGPUMeasured(ctx, specID, index, measured)` for each sample GPU with `VRAMMeasuredMB > 0`. But the write must be skipped when the spec has `VRAMLocked` set (the operator pinned the numbers), and resolving that needs a read by spec ID — which no existing store method provides (`RuntimeSpecByMapping` is keyed by mapping). So **this task adds `RuntimeSpecByID(ctx, id) (RuntimeSpec, bool, error)` to `RuntimeStore`** (SQL + memory + one conformance subtest, following the established shape), reads the spec once per distinct `spec_id` in the sample, and consults `VRAMLocked` before writing. Write-backs are best-effort throughout: log at Debug on error, tolerate `ErrNotFound` for a spec that has since been deleted, and never reject the telemetry sample. Write-backs are best-effort (log Debug on error, never reject the sample).
