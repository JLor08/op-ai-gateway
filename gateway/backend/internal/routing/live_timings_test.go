...
// one of them.
// Closing the hole for the PORTAL half landed in issue #87:
// TestLiveTimingsCapableKindsMatchPortalHandCopy reads
// gateway/frontend/src/components/shared/liveTimings.ts and pins its Set to this
// map -- a same-package test reads the unexported map directly, so no exported
// accessor was needed after all. This size pin stays the cheap mitigation for
// the OTHER sites its message enumerates (docs, i18n, type comments), which no
// test reads.
