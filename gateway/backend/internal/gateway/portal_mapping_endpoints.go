...
var portalMappingErrRows = []errRow{
	...
	{err: portal.ErrMappingCapabilityReserved, status: http.StatusBadRequest, code: "mapping.capability_reserved", msg: `capability_verdicts must not state a verdict for an internally established capability ("live_progress", "speculation_observed"); an empty verdict still resets one`},
