...
const (
	// modulePath is the import prefix of the backend module.
	modulePath = "github.com/your-org/onprem-ai-gateway/gateway/backend"
	// siblingModulePath is the other Go module in this repository. The two
	// modules must stay import-isolated.
	siblingModulePath = "github.com/your-org/onprem-ai-gateway/server-agent"
)
...
// TestInternalPackagesDoNotImportCommandPackages enforces that internal
// packages never depend on the composition roots under cmd/.
func TestInternalPackagesDoNotImportCommandPackages(t *testing.T) {
	for _, pkg := range loadPackages(t) {
		if !strings.HasPrefix(pkg.relDir, "internal/") {
			continue
		}
		for _, imported := range sortedImports(pkg) {
			if strings.HasPrefix(imported, modulePath+"/cmd/") {
				t.Errorf("package %s imports %s: internal packages must not depend on command packages", pkg.relDir, imported)
			}
		}
	}
}
...
// TestModuleBoundaryAgainstSiblingModule enforces that the backend module
// never imports the server-agent module.
...
// TestAuditLogStaysTransportFree enforces that the audit log is a pure
// domain package: it must not depend on any networking package.
func TestAuditLogStaysTransportFree(t *testing.T) {
	for _, pkg := range loadPackages(t) {
		if pkg.relDir != "pkg/auditlog" {
			continue
		}
		for _, imported := range sortedImports(pkg) {
			if strings.HasPrefix(imported, "net/") {
				t.Errorf("package %s imports %s: the audit log must not depend on networking", pkg.relDir, imported)
			}
		}
	}
}
