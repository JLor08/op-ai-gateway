...
// Both locales, not just German (issue #89): run every describe below once per
// locale so the component's English strings are rendered and asserted too.
for (const locale of ['de', 'en'] as readonly Locale[]) {
  const t = messages[locale];

  describe(`ActiveRequestsPanel live metrics columns [${locale}]`, () => {
    ...
    it('names the never-measured provenance in the tooltip', () => {
      render(
        <ActiveRequestsPanel
          ...
          active={[
            makeActive({ tokens_per_second: 0, tokens_per_second_source: '', ttft_ms: 222 }),
          ]}
          effectiveScope="own"
        />,
      );
  ...
  describe(`ActiveRequestsPanel native-passthrough row shapes [${locale}]`, () => {
  ...
  describe(`ActiveRequestsPanel live output-tokens column [${locale}]`, () => {
    it('stays hidden by default and is offered in the column menu', async () => {
      ...
      expect(
        screen.queryByRole('columnheader', { name: t.activityColLiveOutputTokens }),
      ).toBeNull();
