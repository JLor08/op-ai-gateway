...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`MappingSection performance metrics [${locale}]`, () => {
    it('sends all nine metric fields from the create form with distinct values', async () => {
      ...
      fireEvent.change(screen.getByLabelText(t.mappingContextSize), {
        target: { value: '131072' },
      });
    ...
    it('populates every metric field on edit and OMITS the unchanged ones on resubmit', async () => {
      ...
      expect(
        (screen.getByLabelText(t.mappingPromptTokensPerSecond) as HTMLInputElement).value,
      ).toBe('200.25');
      ...
      expect(
        (screen.getByLabelText(t.mappingRecommendedConcurrency) as HTMLInputElement).value,
      ).toBe('8');
      ...
      expect(
        (screen.getByRole('checkbox', { name: t.mappingMetricsLocked }) as HTMLInputElement)
          .checked,
      ).toBe(true);
  ...
  describe(`MappingSection capability verdicts [${locale}]`, () => {
    it('is not undone by the next unrelated save', async () => {
      ...
      await waitFor(() =>
        expect(screen.getByRole('button', { name: t.mappingEdit })).toBeEnabled(),
      );
      fireEvent.click(screen.getByRole('button', { name: t.mappingEdit }));
      expect(screen.getByRole('combobox', { name: t.mappingVisionCapable }).textContent).toBe(
  ...
  describe(`MappingSection status field [${locale}]`, () => {
  ...
  describe(`MappingSection vision column [${locale}]`, () => {
  ...
  describe(`MappingSection manual context-size probe [${locale}]`, () => {
    ...
    it('probes the context size and fills the field (without auto-saving)', async () => {
      ...
      await waitFor(() =>
        expect((screen.getByLabelText(t.mappingContextSize) as HTMLInputElement).value).toBe(
          '8192',
        ),
      );
  ...
  describe(`MappingSection optional metric columns [${locale}]`, () => {
