...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`ApplicationSection health config fields [${locale}]`, () => {
    ...
    it('Custom interval sends the entered seconds on create', async () => {
      ...
      fireEvent.click(
        await screen.findByRole('option', { name: t.applicationHealthIntervalCustom }),
      );
  ...
  describe(`ApplicationSection litellm options [${locale}]`, () => {
  ...
  describe(`ApplicationSection type prefill [${locale}]`, () => {
  ...
  describe(`ApplicationSection server_agent type [${locale}]`, () => {
  ...
  describe(`ApplicationSection non-agent probe fields [${locale}]`, () => {
  ...
  describe(`ApplicationSection reachability indicator [${locale}]`, () => {
  ...
  describe(`ApplicationSection proxy status indicator [${locale}]`, () => {
    ...
    it('shows a distinct excluded chip, not the amber not-proxied one', async () => {
      renderSection({
        apps: [
          makeApp({ id: 'app_1', scheme: 'http', proxy_listen_port: 0, proxy_excluded: true }),
        ],
      });
      await screen.findByText('https://s1.example.test:8000');
  ...
  describe(`ApplicationSection proxy opt-out control [${locale}]`, () => {
  ...
  describe(`ApplicationSection API-variant endpoint modes [${locale}]`, () => {
  ...
  describe(`ApplicationSection path suffix + upstream token [${locale}]`, () => {
  ...
  describe(`ApplicationSection health-interval default display [${locale}]`, () => {
    ...
    it('falls back to the static note when the interval fetch fails', async () => {
      ...
      openCreate();
      expect(
        screen.getByText(t.applicationHealthIntervalNote, { exact: false }),
      ).toBeInTheDocument();
      await waitFor(() => {
  ...
  describe(`ApplicationSection server_agent drill-down + managed_runtime_only [${locale}]`, () => {
  ...
  describe(`ApplicationSection one server_agent application per server [${locale}]`, () => {
    const agentApp = () =>
      makeApp({
        id: 'app_agent',
        type: 'server_agent',
    ...
    it('still shows the type when a pre-invariant server holds two agent applications', async () => {
      ...
      const firstRow = screen
        .getByText('https://s1.example.test:8000')
  ...
  describe(`ApplicationSection managed_runtime_only create button reason [${locale}]`, () => {
    const managedServer: PortalServer = { ...server, managed_runtime_only: true };
    const twoAgents = () => [
      makeApp({ id: 'app_a', type: 'server_agent' }),
      makeApp({
  ...
  describe(`ApplicationSection responses live timings [${locale}]`, () => {
  ...
  describe(`ApplicationSection responses live timings body [${locale}]`, () => {
