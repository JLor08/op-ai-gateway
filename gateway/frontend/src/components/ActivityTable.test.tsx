...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`ActivityTable [${locale}]`, () => {
    ...
    it('renders the owner column (as passed in) respecting the name display', () => {
      ...
      // §6a: the visible label is the user label (activityColOwner), not the
      // owner/table-owner label (tableOwners): "Benutzer" not "Besitzer" in de,
      // "User" not "Owners" in en.
      expect(
        screen.getByRole('columnheader', { name: new RegExp(t.activityColOwner) }),
      ).toBeInTheDocument();
      expect(
        screen.queryByRole('columnheader', { name: new RegExp(t.tableOwners) }),
    ...
    describe('session + agent columns', () => {
      ...
      it('renders an em dash in the session column for a row without a session', () => {
        renderTable({
          columns: sessionColumns,
          rows: [
            makeRow({ session_id: undefined, session_source: undefined, agent_id: undefined }),
          ],
        });
    ...
    describe('service column (Phase 1 service accounts)', () => {
      ...
      it('renders the service name when present', () => {
        renderTable({
          ...
          rows: [makeRow({ service_name: 'Nightly Batch' })],
        });
