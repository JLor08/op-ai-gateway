...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`ServicesView list [${locale}]`, () => {
  ...
  describe(`ServicesView create form [${locale}]`, () => {
    ...
    it('creates a service with the given name, status, allowlist entry, and a full delegate', async () => {
      ...
      const body = (
        fakeApi.createService as unknown as { mock: { calls: [CreateServiceRequest][] } }
      ).mock.calls[0][0];
  ...
  describe(`ServicesView create form — limits [${locale}]`, () => {
    it('submits the entered rate-limit as part of the create request', async () => {
      ...
      fireEvent.change(screen.getByLabelText(t.limitRateRequestsLabel), {
        target: { value: '50' },
      });
  ...
  describe(`ServicesView admin-group picker (Phase C, spec 2026-08-10) [${locale}]`, () => {
    function renderCreate(
      ...
      const create =
        createService ??
        vi.fn(async (body: CreateServiceRequest) =>
    ...
    it('auto-selects the single admin-group candidate (no field) and submits its id', async () => {
      const candidates: AdminGroupCandidate[] = [
        {
          id: 'ag_1',
          name: 'Ops Admins',
          parent_group_id: 'sg_1',
          parent_group_name: 'Ops System',
        },
      ];
      ...
      expect(
        screen.getByText(t.serviceAdminGroupSystemGroupAuto('Ops System')),
      ).toBeInTheDocument();
  ...
  describe(`ServicesView edit-form admin-groups editor (Phase C, spec 2026-08-10) [${locale}]`, () => {
  ...
  describe(`ServicesView detail — settings editable for admin/full-delegate [${locale}]`, () => {
  ...
  describe(`ServicesView detail — read-only for a token-delegate [${locale}]`, () => {
    it('disables every settings field and hides Save, but still shows the token panel', async () => {
      const svc = makeService({
        ...
        delegates: [
          {
            user_id: 'usr_token_delegate',
            user_name: 'Token Delegate',
  ...
  describe(`ServicesView token management [${locale}]`, () => {
  ...
  describe(`ServiceTokensSection unknown-model redirect (Task 8) [${locale}]`, () => {
  ...
  describe(`ServiceTokensSection last-used-model column (Task 9) [${locale}]`, () => {
