...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`GroupsView role-gated sections [${locale}]`, () => {
  ...
  describe(`GroupsView create user-group [${locale}]`, () => {
  ...
  describe(`GroupsView create admin-group for owner (system-admin only) [${locale}]`, () => {
  ...
  describe(`GroupsView delete confirmation [${locale}]`, () => {
  ...
  describe(`GroupsView invitations [${locale}]`, () => {
  ...
  describe(`GroupsView members roster [${locale}]`, () => {
    ...
    it("owner: promotes a plain member, demotes a manager, then transfers ownership to a manager -- each via that row's own action", async () => {
      ...
      await waitFor(() =>
        expect(fakeApi.promoteManager).toHaveBeenCalledWith('ugrp_1', 'usr_plain'),
      );
    ...
    it('a co-manager (not the owner) sees only Entfernen -- never Befoerdern/Degradieren/Eigentuemer wechseln', async () => {
      ...
      expect(
        screen.queryByRole('button', { name: t.groupsActionTransfer }),
      ).not.toBeInTheDocument();
  ...
  describe(`GroupsView per-manager permission controls [${locale}]`, () => {
