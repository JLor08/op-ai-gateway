...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`EdgeCertificatePanel [${locale}]`, () => {
    ...
    it("zeigt einen Hinweis auf 'noch nicht geschrieben', solange written_at fehlt", async () => {
      ...
      renderPanel(api);
      await screen.findByText('edge.example.test');
      expect(screen.getByTestId('cert-edge-delivery')).toHaveTextContent(
        de.certificatesEdgeDeliveryLocalPending('/etc/op-gateway/edge'),
      );
    });
    ...
    it('verspricht KEINE Auslieferung, wenn das Feature aus ist (lokal, pending, aber enabled=false)', async () => {
      ...
      // that promise is false while the feature is off. The whole pending sentence
      // is the discriminating string (there is no i18n key for the phrase alone),
      // so assert the pending message as a whole is absent -- true in both locales.
      expect(deliveryText).not.toHaveTextContent(
        de.certificatesEdgeDeliveryLocalPending('/etc/op-gateway/edge'),
      );
    ...
    it('zeigt die generierte Proxy-Konfiguration im Dialog mit Kopier-Knopf', async () => {
      ...
      expect(
        await screen.findByText(de.certificatesEdgeProxyConfigDialogTitle),
      ).toBeInTheDocument();
      expect(screen.getByTestId('cert-edge-proxy-config-text')).toHaveTextContent(
        'server { # ... }',
      );
      expect(
    ...
    it('speichert Schalter/Aussteller/Namen als eigene, disjunkte PUT-Partition (nur cert_edge_*)', async () => {
      ...
      renderPanel(api);
      const namesField = (await screen.findByLabelText(
        de.settingsCertEdgeNames,
      )) as HTMLInputElement;
    ...
    it("loest 'Jetzt neu ausstellen' erst nach Bestaetigung aus", async () => {
      ...
      renderPanel(api);
      fireEvent.click(
        await screen.findByRole('button', { name: de.certificatesEdgeButtonReissue }),
      );
