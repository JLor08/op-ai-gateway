...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`NetbirdSettings page [${locale}]`, () => {
  ...
  describe(`NetbirdSettings admin-connection gate [${locale}]`, () => {
  ...
  describe(`NetbirdSettings disjoint per-section save [${locale}]`, () => {
  ...
  describe(`NetbirdSettings module-config (url/groups/token/test — Admin section) [${locale}]`, () => {
  ...
  describe(`NetbirdSettings test-without-save + confirm-to-save [${locale}]`, () => {
  ...
  describe(`NetbirdSettings NetBird-only transport (Peer section) [${locale}]`, () => {
    ...
    it('lists gateway-peer options and sends netbird_gateway_peer_id on select', async () => {
      ...
      fireEvent.click(
        await screen.findByRole('button', {
          name: t.settingsNetbirdGatewayPeerChangeConfirmAction,
        }),
      );
      await waitFor(() => expect(updateSystemSettings).toHaveBeenCalled());
      expect(updateSystemSettings.mock.calls[0][0]).toMatchObject({
  ...
  describe(`NetbirdSettings gateway-peer-name field (live display, sticky edit, wish-name save) [${locale}]`, () => {
  ...
  describe(`NetbirdSettings gateway-peer change warning + linked-peer greyout [${locale}]`, () => {
    ...
    it('reloads the gateway-peer dropdown after a save so a renamed peer shows its current name', async () => {
      ...
      const { updateSystemSettings } = renderNetbirdSettings(
        enabledSettings({
          netbird_gateway_peer_id: 'peer-1',
          netbird_gateway_peer_name: 'old-name',
        }),
  ...
  describe(`NetbirdSettings re-enroll confirm (setup-key + sidecar) [${locale}]`, () => {
  ...
  describe(`NetbirdSettings gateway setup key [${locale}]`, () => {
  ...
  describe(`NetbirdSettings sidecar enroll [${locale}]`, () => {
  ...
  describe(`NetbirdSettings policy management [${locale}]`, () => {
  ...
  describe(`NetbirdSettings ping-allow toggles [${locale}]`, () => {
  ...
  describe(`NetbirdSettings token validity + rotation [${locale}]`, () => {
    ...
    it('renders the unknown-validity label when the token status is not known', async () => {
      ...
      // The KNOWN validity line (settingsNetbirdTokenValid) must be absent; derive
      // its locale-specific prefix from t rather than hardcoding the German so it
      // holds in both locales (was a hardcoded `/Token gültig bis/`).
      const tokenValidPrefix = t.settingsNetbirdTokenValid('<DATE>', 0).split('<DATE>')[0];
      expect(
        screen.queryByText((content) => content.startsWith(tokenValidPrefix)),
      ).not.toBeInTheDocument();
    });
