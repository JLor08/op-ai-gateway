...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`SystemSettings theme picker [${locale}]`, () => {
    it('renders theme options by display name (available_themes is [{id,name}])', async () => {
      renderSystemSettings(
        makeSettings({ available_themes: [{ id: 'default', name: 'Default' }] }),
      );
  ...
  describe(`SystemSettings capture_override [${locale}]`, () => {
  ...
  describe(`SystemSettings health_check_interval_seconds [${locale}]`, () => {
  ...
  describe(`SystemSettings agent_presence_timeout_seconds [${locale}]`, () => {
  ...
  describe(`SystemSettings SMTP panel [${locale}]`, () => {
  ...
  describe(`SystemSettings totp_mode [${locale}]`, () => {
  ...
  describe(`SystemSettings route_affinity_session_mode [${locale}]`, () => {
    it('renders both options, defaults to the stored mode, and saves the selected mode', async () => {
      ...
      fireEvent.click(
        await screen.findByRole('option', { name: t.settingsRouteAffinityModeLegacy }),
      );
  ...
  describe(`SystemSettings vision_probe_mode [${locale}]`, () => {
  ...
  describe(`SystemSettings energy defaults [${locale}]`, () => {
  ...
  describe(`SystemSettings currency factor + price unit [${locale}]`, () => {
    ...
    it('round-trips the conversion-factor field into the save payload', async () => {
      ...
      const factorField = (await screen.findByLabelText(
        t.systemCurrencyFactor,
      )) as HTMLInputElement;
  ...
  describe(`SystemSettings NetBird enable checkbox [${locale}]`, () => {
    ...
    it('saves netbird_enabled only — no url/token/groups nor any operational NetBird field', async () => {
      const { updateSystemSettings } = renderSystemSettings(
        makeSettings({ netbird_enabled: false }),
      );
  ...
  describe(`SystemSettings resource_provisioning_enforce (Phase 2, spec 2026-08-12-resource-groups-phase-2-provisioning) [${locale}]`, () => {
