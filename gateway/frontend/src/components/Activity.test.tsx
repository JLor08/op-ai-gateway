...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`Activity self-loading [${locale}]`, () => {
  ...
  describe(`Activity request ordering (reqIdRef guard) [${locale}]`, () => {
  ...
  describe(`Activity energy chart + stat tiles (P3 T2) [${locale}]`, () => {
  ...
  describe(`Activity cost-unit selector (currency-unit T5) [${locale}]`, () => {
    ...
    it('switching the unit reformats both the Usage-list cost column and the aggregate cost tile', async () => {
      ...
      const costColumnCheckbox = await screen.findByRole('checkbox', {
        name: t.activityColCostEur,
      });
  ...
  describe(`Activity collapsible time-series charts [${locale}]`, () => {
  ...
  describe(`Activity group-by chain: legacy activity.groupBy seed vs. an explicit empty chain [${locale}]`, () => {
