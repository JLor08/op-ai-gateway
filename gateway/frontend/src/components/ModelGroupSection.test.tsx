...
const models: ModelOption[] = [
  ...
  { id: 'fast', display_name: 'Fast', flavors: [], loading_on_count: 0, is_group: true },
];
...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`ModelGroupSection [${locale}]`, () => {
    ...
    it('renders the selection controls in the editor', async () => {
      ...
      expect(
        screen.getByRole('option', { name: t.modelGroupMemberOrderSpeed }),
      ).toBeInTheDocument();
    ...
    it('shows the stored selection values for a group loaded from the API', async () => {
      ...
      expect(
        (screen.getByLabelText(t.modelGroupMinTokensPerSecond) as HTMLInputElement).value,
      ).toBe('12.5');
      expect(
        screen.getByRole('combobox', { name: t.modelGroupMinSpeedFallback }),
      ).toHaveTextContent(t.modelGroupMinSpeedFallbackIgnore);
    });
