...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`CertificateSettings [${locale}]`, () => {
  ...
  describe(`CertificateSettings CA last-error alert (F3.1) [${locale}]`, () => {
  ...
  describe(`CertificateSettings scope-flip confirmation (F3.2) [${locale}]`, () => {
    ...
    it("fragt vor dem Speichern nach, wenn Server ohne 'include'-Override betroffen wären", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByRole('combobox', { name: de.settingsCertServerScope }),
        ).toHaveTextContent(de.settingsCertScopeSelected),
      );
    ...
    it("fragt NICHT nach für die Gegenrichtung ('selected' -> 'all'), selbst wenn dieselben Daten in der Vorwärtsrichtung nachfragen würden", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByRole('combobox', { name: de.settingsCertServerScope }),
        ).toHaveTextContent(de.settingsCertScopeAll),
      );
      expect(screen.queryByText(de.certificatesScopeFlipConfirmTitle)).not.toBeInTheDocument();
    });
  });
  ...
  describe(`CertificateSettings diagnostic columns (F3.3) [${locale}]`, () => {
  ...
  // age and mode.
  describe(`CertificateSettings installed column [${locale}]`, () => {
  ...
  describe(`CertificateSettings transport column [${locale}]`, () => {
  ...
  describe(`CertificateSettings mesh require-tls gate (P3) [${locale}]`, () => {
  ...
  describe(`CertificateSettings mesh TLS-port mode (Task 8) [${locale}]`, () => {
    it('renders the mode select with all three options and the read-only port/state', async () => {
      ...
      fireEvent.click(
        screen.getByRole('option', { name: de.certificatesMeshTLSPortModeFollowEnv }),
      );
  ...
  describe(`CertificateSettings public domains panel (U-T4) [${locale}]`, () => {
  ...
  describe(`CertificateSettings https-switch mode + proxy-listen-port base (P4 Task 11) [${locale}]`, () => {
    ...
    it('selecting auto opens a confirm dialog and does NOT PUT yet', async () => {
      ...
      renderCertificateSettings(
        makeApi({
          getSystemSettings: vi.fn(async () =>
            settingsFixture({ cert_https_switch_mode: 'manual' }),
          ),
          updateSystemSettings,
        }),
      );
