...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`UsersView invite dialog [${locale}]`, () => {
    it('widens the invite modal (md) and shows a copy button wired to the invite url', async () => {
      ...
      fireEvent.change(screen.getByLabelText(t.tableEmail), {
    ...
    it('shows an email-sent status line when the backend emailed the link', async () => {
      ...
      const dialog = await screen.findByRole('dialog');
      expect(
        within(dialog).getByText(t.userInviteEmailSent('new@example.com')),
      ).toBeInTheDocument();
    });
  ...
  describe(`UsersView admin TOTP reset [${locale}]`, () => {
    const enrolledUser = {
      id: 'user-1',
  ...
  describe(`UsersView system_admin row actions [${locale}]`, () => {
    const sysAdmin = {
      id: 'sa-1',
  ...
  describe(`UsersView admin limits editor [${locale}]`, () => {
    const plainUser = {
      id: 'user-3',
  ...
  describe(`UsersView invite-form admin-group picker [${locale}]`, () => {
