...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`RuntimeMatrix lower-triangle rendering [${locale}]`, () => {
  ...
  describe(`RuntimeMatrix canonicalisation [${locale}]`, () => {
  ...
  // for the prohibition.
  describe(`RuntimeMatrix cell affordance [${locale}]`, () => {
  ...
  describe(`RuntimeMatrix advisory tooltip [${locale}]`, () => {
  ...
  describe(`RuntimeMatrix disabled (file-mode read-only) [${locale}]`, () => {
  ...
  describe(`RuntimeMatrix rotated column headers [${locale}]`, () => {
  ...
  // ---------------------------------------------------------------------------
  describe(`RuntimeMatrix name ordering [${locale}]`, () => {
  ...
  //     JS `slice(0, N) + '…'` is.
  // ---------------------------------------------------------------------------
  describe(`RuntimeMatrix row-label bounding [${locale}]`, () => {
    ...
    it('never renders two different model names as the same label, on either axis', () => {
      ...
      const rowText = Array.from(container.querySelectorAll('tbody th')).map(
        (th) => th.textContent,
      );
