...
for (const locale of ['de', 'en'] as readonly Locale[]) {
  ...
  describe(`NavSidebar [${locale}]`, () => {
    it('shows labels when expanded and marks the active view', () => {
      ...
      expect(screen.getByRole('link', { name: t.dashboard })).toHaveAttribute(
        'aria-current',
        'page',
      );
    });
    ...
    it('shows the elevated System-Admin-Modus hint above Dashboard, only when elevated', () => {
      ...
      expect(
        hint.compareDocumentPosition(dashboard) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
